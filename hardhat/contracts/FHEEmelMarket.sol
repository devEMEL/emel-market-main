...
// }
        ...
        // a.highestBid = FHE.asEuint64(0);
        // FHE.allowThis(a.highestBid);
        // a.winningAddress = FHE.asEaddress(msg.sender); // Initialize to creator
        // FHE.allowThis(a.winningAddress);

        // a.bids[msg.sender] = FHE.asEuint64(0);
        // FHE.allowThis(a.bids[msg.sender]);
        // FHE.allow(a.bids[msg.sender], msg.sender);
...
contract FHEEmelMarket is SepoliaConfig, ReentrancyGuard, ERC721Holder {
    ...
    function bid(uint256 auctionId, externalEuint64 encryptedAmount, bytes calldata proof) external onlyDuringAuction(auctionId) nonReentrant {
        ...
        // Get encrypted bid and transfer in one block
        euint64 amount = FHE.fromExternal(encryptedAmount, proof);
        ...
        // Update the bid balance for the user
        euint64 previousBid = a.bids[msg.sender];
        if (FHE.isInitialized(previousBid)) {
            // The user increases their bid
            euint64 newBid = FHE.add(previousBid, transferred);
            a.bids[msg.sender] = newBid;
        } else {
            // First bid for the user - add them to bidders array
            a.bids[msg.sender] = transferred;
            a.bidders.push(msg.sender);
        }
        ...
        if (FHE.isInitialized(a.highestBid)) {
            ebool isNewWinner = FHE.lt(a.highestBid, currentBid);
            a.highestBid = FHE.select(isNewWinner, currentBid, a.highestBid);
            a.winningAddress = FHE.select(isNewWinner, FHE.asEaddress(msg.sender), a.winningAddress);
        } else {
